...
class MailSessionTests(unittest.TestCase):
    ...
    def test_code_consumption_rechecks_operator_after_commit(self) -> None:
        token = self.login()
        task_id = self.create_task(token, "mail-code-commit-boundary")
        session = self.create_session(token, task_id)
        self.assertEqual(session.status_code, 201, session.text)
        session_id = session.json()["id"]
        delivered_code = "739154"
        delivered_message_id_hash = hashlib.sha256(
            MESSAGE_ID_HASH_DOMAIN + b"commit-boundary-message"
        ).hexdigest()
        with self.app.state.session_factory() as db:
            persisted = db.get(MailSession, session_id)
        ...
        def commit_then_demote(session_db: Session) -> None:
            ...
            consuming_code = any(
                ...
                and item.event_type == "mail_session.code_consumed"
                for item in session_db.new
            )
            original_commit(session_db)
            if not consuming_code or demoted:
                return
            demoted = True
        ...
        self.assertNotIn(delivered_message_id_hash, response.text)
        with self.app.state.session_factory() as db:
            persisted = db.get(MailSession, session_id)
            events = list(
                db.scalars(
    ...
    def test_waiting_code_status_rechecks_operator_after_audit_commit(self) -> None:
        token = self.login()
        task_id = self.create_task(token, "mail-waiting-status-commit-boundary")
        session = self.create_session(token, task_id)
        self.assertEqual(session.status_code, 201, session.text)
        session_id = session.json()["id"]
        original_commit = Session.commit
        ...
        self.assertEqual(response.status_code, 403, response.text)
        self.assertNotIn("waiting", response.text)
        with self.app.state.session_factory() as db:
            persisted = db.get(MailSession, session_id)
            checked_events = list(
                db.scalars(
    ...
    def test_code_expiry_rechecks_operator_after_commit(self) -> None:
        self.app.state.settings.mail_poll_mode = "worker"
        token = self.login()
        task_id = self.create_task(token, "mail-code-expiry-commit-boundary")
        session = self.create_session(token, task_id)
        self.assertEqual(session.status_code, 201, session.text)
        session_id = session.json()["id"]
        with self.app.state.session_factory() as db:
            ...
            self.assertIsNotNone(persisted)
            persisted.status = "code_ready"
            persisted.delivered_code = "461928"
            persisted.delivered_at = utc_now() - timedelta(minutes=1)
            persisted.delivered_message_id_hash = hashlib.sha256(
                MESSAGE_ID_HASH_DOMAIN + b"expired-code-message"
            ).hexdigest()
        ...
        def commit_then_demote(session_db: Session) -> None:
            ...
            expiring_code = any(
                isinstance(item, AuditEvent)
                and item.event_type == "mail_session.code_expired"
                for item in session_db.new
            )
            original_commit(session_db)
            if not expiring_code or demoted:
                return
            demoted = True
            with self.app.state.session_factory() as other:
                user = other.get(User, self.identity.user_id)
        ...
        self.assertEqual(response.status_code, 403, response.text)
        self.assertNotIn("waiting", response.text)
        self.assertNotIn("461928", response.text)
        with self.app.state.session_factory() as db:
            persisted = db.get(MailSession, session_id)
            expiry_events = list(
                db.scalars(
                    select(AuditEvent).where(
                        AuditEvent.event_type == "mail_session.code_expired",
                        AuditEvent.entity_id == session_id,
                    )
                )
            )
