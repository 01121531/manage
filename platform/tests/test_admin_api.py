...
class AdminApiTests(unittest.TestCase):
    ...
    def test_upload_reconcile_rechecks_actor_after_commit(self) -> None:
        admin_token = self.login(
            "tenant-a",
            "admin@example.test",
        ...
        def demote_after_reconciliation_commit(session: Session) -> None:
            nonlocal admin_demoted
            original_commit(session)
            if reconciliation_recorded and not admin_demoted:
                admin_demoted = True
        ...
        ), mock.patch.object(Session, "commit", new=demote_after_reconciliation_commit):
        ...
        self.assertEqual(reconciled.status_code, 403, reconciled.text)
    ...
    def test_disable_user_rechecks_target_binding_after_cleanup_commit(self) -> None:
        admin_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        def commit_then_move_target(session: Session) -> None:
            ...
            original_commit(session)
            if not cleanup_finished or target_moved:
                return
            target_moved = True
            with self.app.state.session_factory() as other:
                operator = other.get(User, self.operator.user_id)
                self.assertIsNotNone(operator)
                operator.tenant_id = "tenant-b"
                original_commit(other)

        with (
            mock.patch.object(routes, "_revoke_principal_resources", new=revoke_then_mark),
            mock.patch.object(Session, "commit", new=commit_then_move_target),
        ):
            response = self.request(
                "POST",
                f"/api/v1/admin/users/{self.operator.user_id}/disable",
        ...
        self.assertEqual(operator.tenant_id, "tenant-b")
        self.assertFalse(operator.is_active)
        self.assertEqual(disable_event_count, 1)
    ...
    def test_card_enable_rechecks_target_binding_after_commit(self) -> None:
        admin_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        card = provision_card(
            self.app.state.session_factory,
            tenant_id="tenant-a",
            provider_ref="post-commit-moved-enabled-card",
            brand="Visa",
            last4="4242",
            secret_ref="vault://secret/cards/post-commit-moved-enabled",
        )
        disabled = self.request(
            "PATCH",
            f"/api/v1/admin/cards/{card.card_id}",
        ...
        original_record_audit = routes.record_audit
        original_commit = Session.commit
        enable_recorded = False
        card_moved = False

        def mark_enable(*args, **kwargs):
            nonlocal enable_recorded
            result = original_record_audit(*args, **kwargs)
            if kwargs.get("event_type") == "admin.card_enabled":
                enable_recorded = True
            return result

        def commit_then_move_card(session: Session) -> None:
            nonlocal card_moved
            original_commit(session)
            if not enable_recorded or card_moved:
                return
            card_moved = True
            with self.app.state.session_factory() as other:
                current_card = other.get(Card, card.card_id)
        ...
        with self.app.state.session_factory() as db:
            persisted = db.get(Card, card.card_id)
            enabled_audits = db.scalar(
                select(func.count(AuditEvent.id)).where(
                    AuditEvent.event_type == "admin.card_enabled",
                    AuditEvent.entity_id == card.card_id,
                )
            )
        self.assertEqual(persisted.tenant_id, "tenant-b")
        self.assertTrue(persisted.is_active)
        self.assertEqual(enabled_audits, 1)

    def test_quarantined_card_disable_rechecks_target_binding_after_compensation(
        self,
    ) -> None:
        admin_token = self.login(
            "tenant-a",
        ...
        card = provision_card(
            self.app.state.session_factory,
            tenant_id="tenant-a",
            provider_ref="post-compensation-moved-quarantined-state-card",
            brand="Visa",
            last4="4242",
            secret_ref="vault://secret/cards/post-compensation-moved-quarantined-state",
        )
    ...
    def test_card_quarantine_rechecks_target_binding_after_compensation(
        self,
    ) -> None:
        admin_token = self.login(
            "tenant-a",
        ...
        card = provision_card(
            self.app.state.session_factory,
            tenant_id="tenant-a",
            provider_ref="post-compensation-moved-quarantine-card",
            brand="Visa",
            last4="4242",
            secret_ref="vault://secret/cards/post-compensation-moved-quarantine",
        )
        original_compensate = routes._compensate_unavailable_card
        original_commit = Session.commit
        card_moved = False
    ...
    def test_release_card_quarantine_rechecks_actor_after_commit(self) -> None:
        admin_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        card = provision_card(
            self.app.state.session_factory,
            tenant_id="tenant-a",
            provider_ref="post-commit-release-quarantine-card",
            brand="Visa",
            last4="4242",
            secret_ref="vault://secret/cards/post-commit-release-quarantine",
        )
        quarantined = self.request(
            "POST",
            f"/api/v1/admin/cards/{card.card_id}/quarantine",
        ...
        def demote_after_release_commit(session: Session) -> None:
            nonlocal admin_demoted
            original_commit(session)
            if release_recorded and not admin_demoted:
                admin_demoted = True
                with self.app.state.session_factory() as other_db:
                    admin = other_db.get(User, self.admin.user_id)
                    self.assertIsNotNone(admin)
        ...
        ), mock.patch.object(Session, "commit", new=demote_after_release_commit):
        ...
        self.assertEqual(released.status_code, 403, released.text)
        self.assertNotIn("quarantine_reason_code", released.text)
        with self.app.state.session_factory() as db:
            persisted = db.get(Card, card.card_id)
            self.assertIsNotNone(persisted)
            self.assertFalse(persisted.is_active)
    ...
    def test_upload_policy_register_rechecks_tenant_after_commit(self) -> None:
        creator_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        def mark_registration(*args, **kwargs):
            nonlocal policy_id, registration_recorded
            result = original_record_audit(*args, **kwargs)
            if kwargs.get("event_type") == "upload_policy.registered":
                registration_recorded = True
        ...
        def move_policy_after_registration_commit(session: Session) -> None:
            ...
            with self.app.state.session_factory() as other_db:
                policy = other_db.get(UploadPolicyVersion, policy_id)
        ...
        with mock.patch.object(
            ...
            registered = self.request(
                ...
                headers=self.headers(creator_token),
                json={
                    "version": "upload-register-tenant-recheck-v1",
                    "change_note": "verify registration tenant binding",
                },
            )

        self.assertEqual(registered.status_code, 404, registered.text)
        self.assertNotIn("upload-register-tenant-recheck-v1", registered.text)
        with self.app.state.session_factory() as db:
            policy = db.get(UploadPolicyVersion, policy_id)
            registered_audits = db.scalar(
                select(func.count(AuditEvent.id)).where(
                    AuditEvent.event_type == "upload_policy.registered",
    ...
    def test_upload_policy_approve_rechecks_tenant_after_commit(self) -> None:
        creator_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        registered = self.request(
            ...
            headers=self.headers(creator_token),
            json={
                "version": "upload-approve-tenant-recheck-v1",
        ...
        with mock.patch.object(
            ...
            approved = self.request(
                "POST",
                f"/api/v1/admin/policies/upload/versions/{policy_id}/approve",
                headers=self.headers(approver_token),
            )
    ...
    def test_upload_policy_deploy_rechecks_tenant_after_commit(self) -> None:
        creator_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        registered = self.request(
            ...
            headers=self.headers(creator_token),
            json={
                "version": "upload-deploy-tenant-recheck-v1",
        ...
        def move_deployment_after_commit(session: Session) -> None:
            ...
            deployment = session.scalar(
                select(UploadPolicyDeployment).where(
                    UploadPolicyDeployment.tenant_id == "tenant-a"
                )
            )
            original_commit(session)
            ...
            with self.app.state.session_factory() as other_db:
                persisted = other_db.get(UploadPolicyDeployment, deployment_id)
                self.assertIsNotNone(persisted)
        ...
        with mock.patch.object(
            ...
            deployed = self.request(
                "POST",
                f"/api/v1/admin/policies/upload/versions/{policy_id}/deploy",
                headers=self.headers(creator_token),
                json={"rollout_percent": 100},
            )
        ...
        self.assertNotIn("active_version", deployed.text)
        with self.app.state.session_factory() as db:
            deployment = db.get(UploadPolicyDeployment, deployment_id)
            deployed_audits = db.scalar(
                select(func.count(AuditEvent.id)).where(
                    AuditEvent.event_type == "upload_policy.deployed",
                    AuditEvent.entity_id == policy_id,
    ...
    def test_upload_policy_rollback_rechecks_tenant_after_commit(self) -> None:
        creator_token = self.login(
            "tenant-a",
            "admin@example.test",
            "admin-account-password",
        ...
        first_policy_id = register_approve_deploy(
            "upload-rollback-tenant-recheck-v1", 100
        )
        register_approve_deploy("upload-rollback-tenant-recheck-v2", 25)
        with self.app.state.session_factory() as db:
            deployment_id = db.scalar(
                select(UploadPolicyDeployment.id).where(
                    UploadPolicyDeployment.tenant_id == "tenant-a"
                )
            )
        ...
        def move_deployment_after_rollback_commit(session: Session) -> None:
            ...
            deployment_moved = True
            with self.app.state.session_factory() as other_db:
                deployment = other_db.get(UploadPolicyDeployment, deployment_id)
