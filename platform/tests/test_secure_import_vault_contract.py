...
class SecureImportVaultContractTests(unittest.TestCase):
    ...
    def test_roles_are_pool_isolated_and_api_cannot_sign(self) -> None:
        ...
        self.assertEqual(contract["schema_version"], 38)
        ...
        self.assertEqual(
            contract["ingestion_boundary"],
            {
                ...
                "card_batch_identity": {
                    "field": "provider_ref",
                    "comparison": "exact_after_existing_field_normalization",
                    "duplicates": "rejected",
                    "cli_validation": (
                        "before_platform_token_context_vault_or_local_evidence_use"
                    ),
                    "api_validation": (
                        "before_receipt_verification_context_verification_or_"
                        "database_write"
                    ),
                },
                "pools": {
            ...
            contract["target_import_context"],
            {
                ...
                "card_identity_claims": {
                    ...
                    "reclamation_candidate_lock": (
                        "matching_expired_context_rows_for_update_before_claim_transfer"
                    ),
                    "reclamation_lock_order": (
                        "ascending_context_id_before_claim_read_and_transfer"
                    ),
                    "claim_insert_order": (
                        "ascending_provider_ref_with_manifest_position_preserved"
                    ),
                    "delete_policy": (
                        "database_forbidden_reclamation_transfers_existing_claim"
                    ),
                    "identity_mutation_policy": (
                        "database_forbidden_tenant_and_provider_ref_after_insert"
                    ),
                    "claim_mutation_ledger": (
                        "database_generated_append_only_context_and_position_"
                        "transition_evidence_without_provider_ref_or_card_secret"
                    ),
                    "reclamation_audit": (
                ...
                "consumption_record_mutation_policy": (
                    "database_append_only_after_insert"
                ),
                "consumption_lifecycle_policy": (
                    "database_one_way_exact_receipt_bound_terminal"
                ),
            },
        )
