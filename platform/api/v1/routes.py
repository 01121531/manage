...
def replace_card_allocation(
    ...
    try:
        ...
        if replay is not None:
            ...
            replacement_link = db.scalar(
                select(CardAllocationReplacement)
                .where(
...
def release_card_allocation(
    ...
    allocation = db.scalar(
        ...
        .where(
            ...
            CardAllocation.status == "released",
            CardAllocation.released_at.is_not(None),
            CardAllocation.release_reason_code == "user_released",
        )
        .with_for_update()
        .execution_options(populate_existing=True)
    )
...
def admin_disable_user(
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    if actor.role != ROLE_PLATFORM_ADMIN and user.role == ROLE_PLATFORM_ADMIN:
        raise HTTPException(status_code=403, detail="Cannot disable a platform administrator")
    if user.is_active:
        user.is_active = False
        record_audit(
