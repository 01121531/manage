...
test('ops admin reads masked card history and recycles one exact allocation', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (
      ...
      return fulfill({ ...allocation, id: 'wrong-allocation-binding' })
    }
    return fulfill({ error: { code: 'not_found', message: 'not found' } }, 404)
  })
  ...
  await expect(page.locator('body')).not.toContainText('wrong-allocation-binding')
...
test('ops admin safely reconciles unknown uploads with true-state recovery', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (cancelMatch && request.method() === 'POST') {
      ...
      return fulfill({ ...uploads[0], id: 'wrong-upload-binding', status: 'cancelled' })
    }
    return fulfill({ error: { code: 'not_found', message: 'not found' } }, 404)
  })
  ...
  try {
    ...
    await expect(page.locator('body')).not.toContainText('wrong-upload-binding')
