...
- Repository-external validator source-snapshot path, 92-member exact-tree/read-only result, caller-retained manifest payload/file SHA-256 pins, and pre/post identity/byte recheck evidence:
- Clean-child launch result: absolute interpreter, parent-captured interpreter SHA-256, exact `-I -B -S -P`, sanitized environment, snapshot cwd, local-module origin/digest audit, interpreter/runtime pre/post recheck, direct operational entrypoint blocked, and recovery snapshot mutation not performed:
- Generation-context external-handoff closed-v1 synthetic policy SHA-256 and pending readiness-template verification result, including exact subject bindings, three dedicated role/domain/scope declarations, timestamp nonce/imprint requirements, provider immutable-version/CAS/read-after-head requirements, and no-write/no-network result:
- Generation-context handoff acknowledgement: policy is `unconfigured`, readiness is `pending`, authoring/recovery do not consume it, no generation publication was performed, and policy-pin authority, context signer authentication/authorization, anchor validity/revocation, trusted timestamp/replay protection, provider-native head/CAS, global fork protection and rollback protection remain `unverified`:
- Runtime bundle/image external-attestation closed-v1 synthetic policy SHA-256 and pending readiness-template verification result, including exact generation/runtime/deployment/target subject bindings, five isolated role/domain/scope declarations, immutable digest-only selection, publisher trust/revocation, provenance, target process/loaded evidence, timestamp nonce/imprint and provider CAS/read-after-head requirements, plus no-write/no-network/no-signing result:
- Runtime-attestation handoff acknowledgement: policy is `unconfigured`, readiness is `pending`, authoring/recovery/deployment/runtime acceptance do not consume it, no repository signature, provider mutation or target observation was performed, and publisher authentication/authorization, anchor validity/revocation freshness, provenance, immutable artifact version, deployment digest selection, target runtime/process/loaded evidence, trusted timestamp/replay protection, provider-native head/CAS, global fork/rollback protection, runtime authority and original execution remain `unverified`:
- Runtime-attestation configured offline-fixture profile and evidence raw SHA-256 pins, independent runtime-subject canonical SHA-256 pin, canonical-bytes/closed-schema result, Sigstore v0.3 and GitHub SLSA v1 format result, deployment/target/timestamp/head cross-binding result, and pure-bytes/no-I/O result:
- Runtime-attestation protocol-only acknowledgement: the fixture carries synthetic unverified provider records, `hermetic_build_claim=false`, is not consumed by authoring/recovery/deployment/runtime acceptance, and does not authenticate publisher/provenance, trust-root or revocation currentness, target observer, trusted time, provider CAS/head, global fork/rollback protection, runtime authority, original execution or production acceptance:
- Full genesis-to-terminal generation-receipt chain result, including exact predecessor manifest/receipt selectors, sequence, registered item/artifact digest, exactly-one missing-to-provided transition replay, each generation's embedded requirements/matrix, recorded host-UTC and validator-contract semantic replay, final stable single-link recheck and receipt-as-acceptance-point acknowledgement:
...
- Schema-v2 finalization receipt self-bound locator, recorded finalization evaluation instant, exact terminal-generation/snapshot/checkpoint/final-leaf binding, Phase 0 ancestor result, local no-replace/readback, orphan/unknown disposition, and receipt-authority/parent-directory-race/publication-crash-durability/post-publication-custody `unverified` acknowledgement:
- Whole receipt/result/four-pin rollback, same-path same-bytes delete/recreate and locator-continuity `unverified` acknowledgement:
